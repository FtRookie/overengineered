...
class WordMemoryEditorRow extends Control<MemoryEditorRecordDefinition> {
    ...
    constructor(
        ...
        this.onEnable(() => {
            // Адрес строки отображается в HEX
            ...
            for (let i = 0; i < 16; i++) {
                ...
                if (currentVal !== undefined) {
                    // value хранится как обычный number,
                    // WordTextBoxControl сам показывает его как HEX.
                ...
                control.submitted.Connect((value) => {
                    ...
                    // если пользователь записал значение дальше по памяти.
                    for (let j = 0; j < cellIndex; j++) {
                        this.popup.data[j] ??= 0;
                    }

                    // Здесь value уже обычный number.
                    // Например FFFF -> 65535.
    ...
    private updateAsciiLabel() {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            // ASCII берём из младшего байта слова.
    ...
    updateColor(columnIndex: number) {
        ...
        if (controls[columnIndex]) {
            ...
            controls[columnIndex].instance.TextColor3 =
                this.popup.data[cellIndex] !== undefined
                    ? Colors.white
                    : Color3.fromRGB(180, 180, 180);
        }
    }
...
class WordMemoryEditorRows extends Control<MemoryEditorRecordsDefinition> {
    ...
    constructor(
        ...
        this.rows = this.parent(
            new ComponentChildren<WordMemoryEditorRow>().withParentInstance(this.gui),
        );

        this.gui.GetPropertyChangedSignal("CanvasPosition").Connect(() => {
            const onStart =
                VectorUtils.roundVector2(this.gui.CanvasPosition).Y === 0;

            const onEnd =
                VectorUtils.roundVector2(
                    this.gui.AbsoluteCanvasSize.sub(this.gui.CanvasPosition),
                ).Y ===
        ...
        const loadBehind = () => {
            ...
            this.gui.CanvasPosition = this.gui.CanvasPosition.add(
                new Vector2(
                    0,
                    this.gui.Template.Size.Y.Offset *
    ...
    spawnRows() {
        ...
        for (let i = 0; i < this.contentSize; i++) {
            ...
            const rowControl = new WordMemoryEditorRow(
                this.template(),
                this.popup,
                row,
                (targetCellIndex) => {
...
export class MemoryEditor16BitPopup extends Control<WordMemoryEditorPopupDefinition> {
    ...
    constructor(
        ...
        const rows = this.parent(
            new WordMemoryEditorRows(gui.Content, this),
        );
        ...
        // Адрес текущей строки при скролле.
        // В GUI отображается HEX, внутри остаётся обычный number.
        gui.Content.GetPropertyChangedSignal("CanvasPosition").Connect(() => {
            const rowHeight =
                gui.Content.Template.Size.Y.Offset * this.getScale();

            if (rowHeight === 0) return;

            const currentRow =
                rows.rowCursor +
                math.round(gui.Content.CanvasPosition.Y / rowHeight);

            gui.AddressTextBox.Text = string.format(
                "%04X",
                currentRow * 16,
            );
        });
                    ...
                    new TextPopup(
                        ...
                        (text) => {
                            ...
                            for (const [tokenRaw] of text.gmatch("%S+")) {
                                ...
                                // Поддерживаем необязательный 0x / 0X.
                                const [withoutPrefix] =
                                    token.gsub("^0[xX]", "");

                                // Оставляем только HEX-символы.
                                const [cleanHex] =
                                    withoutPrefix.gsub("[^%dA-Fa-f]", "");
                            ...
                            if (
                                words.size() === 0 ||
                                words.size() > this.wordsLimit
                            ) {
                                LogControl.instance.addLine(
                                    "Invalid data size for import!",
                                    Colors.red,
                                );
                                return;
                            }
        ...
        // Адреса вводятся в HEX.
        // TextBoxControl здесь остаётся обычным текстовым полем,
        // потому что нам нужно самостоятельно преобразовать HEX -> number.
        ...
        addressTextBox.submitted.Connect((value) => {
            ...
            if (targetAddress !== undefined) {
                ...
                const maxCursor = math.max(
                    0,
                    totalRows - rows.contentSize,
                );

                const cursorRow = math.clamp(
                    clampedRow,
                    0,
                    maxCursor,
                );
                ...
                const rowHeight =
                    gui.Content.Template.Size.Y.Offset;

                const visualRowOffset =
                    clampedRow - rows.rowCursor;

                gui.Content.CanvasPosition = new Vector2(
                    0,
                    math.max(
                        0,
                        visualRowOffset * rowHeight * scale,
                    ),
                );
    ...
    getScale() {
        return this.parentScreen
            .getComponent(AutoUIScaledComponent)
            .getScale();
    }
}