...
- **The ideal is zero comments — code that explains itself needs none.** A comment exists only to fill a gap where the meaning is *not* obvious from the code; it is a confession that the code could not carry it alone, not a default to reach for. Aim for none and add one only where the code genuinely cannot speak. That bar is met by magic values (where a constant came from), maths, and engine quirks that would otherwise read as a mistake (`// NaN fails self-compare; ±inf fail the bounds` above a `n === n` check in `PlacementValidation.ts`) — plus the occasional key or name that no longer conveys its purpose (`//a.k.a. rewrite value` in `CounterBlock.ts`). Everything else, leave out. Keep what survives to one line and to the bare minimum for surface-level understanding: a reader who needs more detail reads the code. **Grammar is expendable, clarity is not.** Drop articles, use fragments, write like a note rather than prose — `no immediate resume, runs next tick, assert non null` — and stop trimming only where the next cut would make it ambiguous. Never narrate what the code does (`//set value`), and trim a comment that has grown longer than the logic it guards. There is no target density to hit — fewer is always better.
    ...
    **Three forms, escalating — reach for the smallest that fits.**
    ...
    - **JSDoc** (`/** */`) is for a frequently-called API whose callers should read its intent without opening the implementation. Common on `engine/` (~70% of files), rare in game code (~20% of `shared/blocks`).
